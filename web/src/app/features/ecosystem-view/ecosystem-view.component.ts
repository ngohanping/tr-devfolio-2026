...
export class EcosystemViewComponent implements OnInit {
  ...
  readonly SECTION_GAP = 150;  // gap between projects row and services section
  ...
  private layoutNodes(nodes: EcosystemNode[]): void {
    ...
    this.projectNodes = projects.map((p, i) => {
      const serviceCount = 0; // will be calculated from edges
      return {
        ...p,
    ...
    // Layout services in bottom section (grouped by type)
    ...
    // Count services per project for badge
  ...
  private prepareEdges(rawEdges: EcosystemEdge[]): EcosystemEdge[] {
    // Build member-to-merged-service map
    ...
    for (const edge of rawEdges) {
      ...
      // Remap service IDs to merged nodes
      ...
      if (edge.type === 'CALLS' || edge.type === 'EXPOSED_BY') {
        ...
        // Project → Service: remap service to merged node
        tgt = memberToMerged.get(tgt) ?? tgt;
      }
